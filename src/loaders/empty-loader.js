// returns the base-level package folder based on detecting "node_modules" package name boundaries
const pkgNameRegEx = /^(@[^\\\/]+[\\\/])?[^\\\/]+/;
  ...
  if (pkgBase) {
    ...
    if (baseParts[baseParts.length - 2] === "node_modules") {
      ...
      if (pkgName in emptyModules) {
        console.warn(
          `furled: Ignoring build of ${pkgName}, as it is not statically analyzable. Build with "--external ${pkgName}" if this package is needed.`
        );
        return "";
      }
    }
